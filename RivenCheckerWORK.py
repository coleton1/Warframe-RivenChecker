        ...
        if response.status_code == 200:
            ...
            if attacks:
                for index, attack in enumerate(attacks, start=1):
                    ...
                    crit_chance_label = ctk.CTkLabel(root4, text=f"{attack_name} - Crit Chance: {crit_chance}",font=("Arial", 11, "bold"))
                    crit_chance_label.pack(pady=1,anchor='w')
...
def display_baro():
    ...
    if root3 is None or not root3.winfo_exists():
        ...
        if response.status_code == 200:
            ...
            bg_image_path = os.path.join(os.path.dirname(__file__), 'baro.png')
            bg_image = PIL.Image.open(resource_path(bg_image_path))
            bg1_image = CTkImage(light_image=bg_image, size=(700, 400))
...
bg_image = PIL.Image.open(resource_path(bg_image_path))
bg1_image = CTkImage(light_image=bg_image, size=(250,200))
bg_label = CTkLabel(root, text="", image=bg1_image)
bg_label.place(relwidth=1, relheight=1.6)
bg_label.text = ""
